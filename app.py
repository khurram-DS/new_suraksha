...
def main():
    ...
    @st.cache_data
    def load_ward_data():
        df = pd.read_excel('complete_ward_data.xlsx')
        return df

    # Load Suraksha Coordinates Data
    @st.cache_data
    def load_suraksha_data():
        df = pd.read_csv('Suraksha_coordinates.csv')
        return df

    population_df = load_ward_data()
    suraksha_df = load_suraksha_data()

    # Data Source Information Dropdown
    data_info = {
        "Census Data 2011": "-- Loaded from the Census of India website. Contains population and demographic details by ward.",
        "Ward Details": "-- Scraped from Wikipedia. Includes latitude, longitude, and pin codes for each ward in KMC and BMC.",
        "Suraksha Locations": "-- Addresses from Suraksha website; coordinates from Google Maps. scraped from google maps",
        "Income Data": "-- Based on house rent rates, data scraped from Housing.com for individual wards. Taking average Data of house rent with avg sq ft area. considered rule of thumb like people use to invest there 30% of there income in rent which means adding that 70% income to the required house rent give me required house rent another paramter is Adjusted required parameter which is based on weighing factors of other parameter like population, literacy rate,working population and other data.)",
        "Amenities Data": "-- Data about nearby amenities scraped using Open Street Map and Google API's , so we have scraped all the nearby healthcare data, and the busiest place and also find there reviews and ratings to understand the people response."
    }

    choice = st.selectbox("**Select Data Information**", list(data_info.keys()))
    
    # Displaying the selected data information
    st.info(data_info[choice])

    # Link to PDF
    pdf_path = 'static/hierarchy.pdf'
    if os.path.exists(pdf_path):
        pdf_link = f'<a href="data:application/pdf;base64,{base64.b64encode(open(pdf_path, "rb").read()).decode()}" target="_blank">View Hierarchy for this project and process</a>'
    ...
    # Create a dropdown to select the feature for distribution plots
    ...
    if feature == 'total_population':
        st.plotly_chart(create_histogram(population_df, 'total_population', 'Distribution of Total Population'))
    elif feature == 'required_annual_income':
        st.plotly_chart(create_histogram(population_df, 'required_annual_income', 'Distribution of Required Annual Income'))
    elif feature == 'adjusted_required_annual_income':
        st.plotly_chart(create_histogram(population_df, 'adjusted_required_annual_income', 'Distribution of Adjusted Required Annual Income'))
    elif feature == 'literacy_rate':
        st.plotly_chart(create_histogram(population_df, 'literacy_rate', 'Distribution of Literacy Rate'))
    ...
    st.subheader("➡️ Finding the corelation with the target variable 'suraksha_center_within_3km'Trying to check existing suraksha center to understand , what features are most related to our existing center")
    # Define the new key features including the target variable
    key_features = [
    ...
    # Function to check if there is a Suraksha center within 3 km and count them
    ...
    # Apply the function to calculate the number of nearby Suraksha centers for each ward
    population_df['suraksha_center_within_3km'] = population_df.apply(lambda row: count_nearby_suraksha_centers(row['lat'], row['long'], suraksha_df), axis=1)
    
    # Define the new key features including the target variable
    key_features_with_target = key_features + ['suraksha_center_within_3km']
    
    # Calculate the correlation matrix for key features with the new target variable
    ...
    # Calculate correlations with the new target variable
    ...
    # Categorize features into slabs
    ...
    # Generate detailed summary for each slab
    def generate_detailed_summary(features, correlation_matrix, target_variable, slab_name):
        ...
        for feature in features:
            ...
            # Meaning and suggestion based on feature
            if feature == 'total_population':
                meaning = "the total number of people living in the ward"
                suggestion = "Suraksha centers are not necessarily placed in areas with higher populations."
                potential_improvement = "Consider placing centers in areas with a higher need for healthcare services rather than just high population."
            elif feature == 'total_worker_population':
                meaning = "the total number of working individuals in the ward"
                suggestion = "Suraksha centers are likely placed in areas with higher working populations due to economic activity."
                potential_improvement = "Ensure that healthcare services cater to the working population's needs, such as extended hours and quick service."
            elif feature == 'price_sq_ft':
                meaning = "the price per square foot in the ward"
                suggestion = "Higher property prices often indicate a higher standard of living and economic activity."
                potential_improvement = "Target areas with moderate property prices to balance accessibility and profitability."
            elif feature == 'annual_rent':
                meaning = "the annual rent in the ward"
                suggestion = "Higher annual rents suggest a higher cost of living."
                potential_improvement = "Consider affordability for patients when choosing locations."
            elif feature == 'literacy_rate':
                meaning = "the literacy rate in the ward"
                suggestion = "Higher literacy rates may correlate with better awareness and usage of healthcare services."
                potential_improvement = "Focus on health education and awareness campaigns in areas with lower literacy rates."
            # Add more feature-specific meanings, suggestions, and potential improvements as needed
            else:
                meaning = "an unspecified feature"
                suggestion = "no specific suggestion for this feature."
                potential_improvement = "Further investigate this feature to understand its impact."
    
            summary.append(f"**Feature:** '{feature}'\n"
                           f"**Correlation:** {correlation_value:.2f} ({relationship} correlated)\n"
                           f"**Meaning:** This represents {meaning}.\n"
                           f"**Implication:** This indicates that {suggestion}\n"
                           f"**Potential Improvement:** {potential_improvement}\n")
        
        return "\n".join(summary)
    
    # Generate summaries for each slab
    ...
    # Display the summaries in collapsible sections
    ...
    # Define highly correlated features
    ...
    population_df['dbscan_cluster'] = dbscan.fit_predict(features_scaled)
    
    dbscan_labels = population_df['dbscan_cluster']
    dbscan_labels_filtered = dbscan_labels[dbscan_labels != -1]
    dbscan_features_filtered = features_scaled[dbscan_labels != -1]
    ...
    st.markdown('**Results for clusters**')
    ...
    cluster_summary = population_df.groupby('kmeans_cluster').agg({
        'total_population': ['mean', 'std', 'min', 'max'],
        'total_population_males': ['mean', 'std', 'min', 'max'],
        'total_population_females': ['mean', 'std', 'min', 'max'],
        'person_aged_0_6': ['mean', 'std', 'min', 'max'],
        'literate_population': ['mean', 'std', 'min', 'max'],
        'total_worker_population': ['mean', 'std', 'min', 'max'],
        'non_working_population': ['mean', 'std', 'min', 'max'],
        'literacy_rate': ['mean', 'std', 'min', 'max'],
        'Healthcare Service Count': ['mean', 'std', 'min', 'max'],
        'Busiest Place Count': ['mean', 'std', 'min', 'max'],
        'Avg Healthcare Rating': ['mean', 'std', 'min', 'max'],
        'Avg Busiest Place Rating': ['mean', 'std', 'min', 'max'],
        'healthcare_sentiment_score': ['mean', 'std', 'min', 'max'],
        'busiest_place_sentiment_score': ['mean', 'std', 'min', 'max']
    }).reset_index().round(2)
    ...
    # Streamlit app
    #st.title("Suraksha Diagnostic Center Location Analysis")
    
    # Button to display the map
    if st.button('Show Map'):
        st.plotly_chart(fig_top_candidates)
    
    # Dropdown to select top candidates
    ...
    # Download button for final candidates data
    st.download_button(
    ...
    colormap = plt.cm.tab20  # Using the recommended syntax for colormap
